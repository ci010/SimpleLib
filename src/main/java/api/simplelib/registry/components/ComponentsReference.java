...
public @interface ComponentsReference
{
	 ...
	 * <p>If there are </p>
	 * Inject a Block/Item/ComponentStruct reference by finding the include getName.
	 */
	@Retention(RetentionPolicy.RUNTIME)
	@Target(value =
			{ElementType.FIELD})
	...
	{
		/**
		 * @return The register getName of the component. The default format should be like
		 * {@link net.minecraft.util.ResourceLocation}: domain:getName. The domain is the mod id of the component.
		 * <p>If it
		 * owned by Minecraft, The domain should be minecraft.</p>
		 * <p>If you simply put the register getName here. The domain will be the the {@link ComponentsReference#domain()}</p>
		 */
		String value();
	}
}
