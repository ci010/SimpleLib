...
{
	/**
	 * Return a {@link net.minecraftforge.common.capabilities.Capability.IStorage} which just does nothing.
	 *
	 * @return A empty {@link net.minecraftforge.common.capabilities.Capability.IStorage}.
	 */
	public static <T> Capability.IStorage<T> emptyStorage() {return TypeUtils.cast(STORE);}
	...
	public static <BuilderType> ICapabilityProvider createFromCustomBuilder
			(Class<BuilderType> type, ItemStack stack, Consumer<BuilderType> predicate)
	{
		return createFromCustomBuilder0(type, stack, predicate);
	}

	public static <BuilderType> ICapabilityProvider createFromCustomBuilder
			(Class<BuilderType> type, TileEntity tile, Consumer<BuilderType> predicate)
	{
		return createFromCustomBuilder0(type, tile, predicate);
	}

	public static <BuilderType> ICapabilityProvider createFromCustomBuilder
			(Class<BuilderType> type, Entity entity, Consumer<BuilderType> predicate)
	{
		return createFromCustomBuilder0(type, entity, predicate);
	}

	private static <BuilderType> ICapabilityProvider createFromCustomBuilder0
			(Class<BuilderType> type, Object object, Consumer<BuilderType> predicate)
	{
		CapabilityBuilderHandler<BuilderType> handler = CapabilityBuildManager.instance().getBuilderHandler(type);
		BuilderType builder = handler.createBuilder(object);
		predicate.accept(builder);
		return handler.build(builder, object);
	}
	...
	public static Map<ResourceLocation, ICapabilityProvider> revolve(Entity entity)
	{
		return CapabilityBuildManager.instance().handle(entity);
	}

	public static Map<ResourceLocation, ICapabilityProvider> revolve(ItemStack stack)
	{
		return CapabilityBuildManager.instance().handle(stack);
	}

	public static Map<ResourceLocation, ICapabilityProvider> revolve(TileEntity tileEntity)
	{
		return CapabilityBuildManager.instance().handle(tileEntity);
	}
	...
	public static class Sided<T> implements ICapabilityProvider
	{
		...
		{
			this.capability = capability;
			sidedInstances = ArrayUtils.newArrayWithCapacity(EnumFacing.values().length);
		...
		public <C> C getCapability(Capability<C> capability, EnumFacing facing)
		{
			return TypeUtils.cast(sidedInstances[facing.ordinal()]);
		}
	}
