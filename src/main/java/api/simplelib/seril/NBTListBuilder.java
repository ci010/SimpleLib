...
public class NBTListBuilder<T> implements List<T>
{
	...
	{
		if (!NBTBases.instance().isBaseType(type) && !ICompoundSerializable.class.isAssignableFrom(type))
			throw new IllegalArgumentException("Not support " + type + " for nbt list");
		return new NBTListBuilder<T>(type);
	}

	public static <T> NBTListBuilder<T> create(Class<T> type, INBTSerializer<T> serializer)
	{
	...
	public static <T> NBTListBuilder<T> create(NBTTagList list, INBTSerializer<T> serializer)
	{
	...
	NBTListBuilder<T> setSerial(INBTSerializer<T> serial)
	{
	...
	private NBTListBuilder(Class<T> type)
	{
		...
		serializable = ICompoundSerializable.class.isAssignableFrom(type);
		if (serializable)
...
//		}
	...
	public NBTTagList build()
	{
		...
		for (T t : this.list)
			...
			{
				ICompoundSerializable serializable = (ICompoundSerializable) t;
