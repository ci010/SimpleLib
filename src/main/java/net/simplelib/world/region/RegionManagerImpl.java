...
import static api.simplelib.seril.NBTBasement.STRING;
...
public class RegionManagerImpl implements RegionManager
{
	...
	@Override
	public Optional<RegionImpl> newRegion(String id, int x1, int z1, int x2, int z2)
	{
	...
	protected boolean onRegionExpand(RegionImpl region, List<Chunk> chunks, Rectangle2D area)
	{
		for (Chunk chunk : chunks)
		{
			...
			{
				RegionImpl r = this.idToRegion.get(string);
				if (r.intersect(area))
				{
					//Log
					return false;
	...
	@Override
	public void deleteRegion(RegionImpl region)
	{
		region.removeAll();
