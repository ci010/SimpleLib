	...
	public final void load(ASMDataTable table)
	{
		...
		for (ASMDataTable.ASMData data : table.getAll(Mod.class.getName()))
		{
			try
			{
				...
				rootMap.put(classSafe.getPackage().getName(), data.getAnnotationInfo().get("modid").toString());
			}
		...
		for (ASMDataTable.ASMData asmData : table.getAll(API.class.getName()))
		{
			System.out.println(asmData.getClassName());
			Iterable<String> itr = Splitter.on('.').split(asmData.getClassName());
			String[] split = Iterators.toArray(itr.iterator(), String.class);
			split = ArrayUtils.remove(split, split.length - 1);
			rootMap.put(Joiner.on('.').join(split), (String) asmData.getAnnotationInfo().get("simple_lib"));
		}
			...
			for (Method method : registryDelegateType.getDeclaredMethods())
			{
				...
				if (asmType)
				{
					...
					for (ASMDataTable.ASMData meta : list)
					{
						...
						String modid = rootMap.getModid(pkg);
						if (modid == null)
						{
...
//							HelperMod.metadata.childMods.add();
	...
	public String getModidByPackage(Package pkg) {return rootMap.getModid(pkg);}

	public String getModidByPackage(Class<?> clz) {return rootMap.getModid(clz);}
