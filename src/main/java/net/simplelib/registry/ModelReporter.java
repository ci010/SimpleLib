...
//	private static ModelResourceLocation MISS = new ModelResourceLocation("builtin/missing", "missing");
	...
	public void onModelPost(ModelBakeEvent event)
	{
		...
		Set<ModelResourceLocation> set = ReflectionHelper.getPrivateValue(ModelLoader.class, event.getModelLoader(),
				"missingVariants");
		IRegistry<ModelResourceLocation, IBakedModel> modelReg = event.getModelRegistry();
//		Object missingModel = modelReg.getArgs(MISS);
		...
		try
		{
			for (final ModelStates state : list)
				...
				else this.writeBlock(state, gson);
		}
		catch (IOException e)
		{
			e.printStackTrace();
	...
	{
		Environment refer = Environment.getBean(state.domain());
		File modelItem = new File(refer.getItemModelDirectory(), state.path().concat(".json"));
		if (standard != null)
			FileUtils.write(new File(refer.getItemModelDirectory(), "standard.json"), gson.toJson(standard));
		standard = null;
	...
	{
		Environment refer = Environment.getBean(state.domain());
		File stateFile = new File(refer.getBlockStateDirectory(), state.path().concat(".json"));
		File modelItem = new File(refer.getItemModelDirectory(), state.path().concat(".json"));
		...
		{
			File modelFile = new File(refer.getBlockModelDirectory(), s.concat(".json"));
