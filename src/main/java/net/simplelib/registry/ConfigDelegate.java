...
public class ConfigDelegate extends ASMRegistryDelegate<ModConfig>
{
	...
	public void init(FMLPreInitializationEvent event)
	{
		...
		String modid = modConfig.domain().equals("") ? this.getModid() : modConfig.domain();
		Configuration cfg = new Configuration(event.getModConfigurationDirectory(), modid);
	...
	{
		String id = modConfig.id().equals("") ? f.getName() : modConfig.id();
		...
		int maxLength;
		String comment = parseComment(modConfig, id);
		...
		{
			ModConfig.FixLength annotation = f.getAnnotation(ModConfig.FixLength.class);
			if (annotation == null)
			{
				throw new IllegalArgumentException();
			}
		...
		Property property = cfg.get(modConfig.categoryId(), id,
				defaultValue,
				comment,
				propType);
	...
	private String parseComment(ModConfig modConfig, String id)
	{
		String comment = modConfig.comment();
		if (comment.equals(""))
		{
			String unloc = Joiner.on('.').join("cfg", modConfig.categoryId(), id);
