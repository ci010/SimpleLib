...
public class LanguageReporter
{
	...
	{
//		if (!unlocalizedName.endsWith(".getName"))
//			unlocalizedName = unlocalizedName.concat(".getName=");
//		else
	...
	public LanguageReporter start(String modid, String[] str)
	{
		try
		{
			for(String name : str)
			{
				File f = new File(Environment.getBean(modid).getLanguageDirectory(), name.concat(".lang"));
				if (!f.exists())
					f.createNewFile();
				fileLang.add(f);
			}
